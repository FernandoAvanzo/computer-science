...
def validade_progs(path, progs):
    if not progs:
        print
        u'Nenhum arquivo run executável encontrado em', path
        print
        u'Lembre-se de executar o comando "chmod -f +x src/lang/{run,build}"'
        print
        u'onde lang é a linguagens escolhida para implementar o trabalho.'
        sys.exit(1)
...
def validade_cases(path, cases):
    if not cases:
        print
        u'Nenhum arquivo de teste encontrado em', path
        sys.exit(1)


def show_usage():
    print
    u'Modo de usar: %s cases-path progs-path %s' % (sys.argv[0], get_formatted_checkers())


def main():
    if len(sys.argv) != 4:
        print
        u'Número inválido de parâmetros'
        show_usage()
        sys.exit(1)
    cases_path, progs_path, alg = sys.argv[1:]
    if alg not in CHECKERS:
        print
        u'Algoritmo inválido:', alg
        show_usage()
        sys.exit(1)
